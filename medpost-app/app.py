...
def fetch_articles(selectedfeed, newspaper):
    if selectedfeed == "tous":
        ...
        subquery = (
            db.session.query(Posts.id_article)  # Articles postés sur selectedfeed
...
def fetch_planned_posts(selectedfeed, newspaper):
    ...
    if selectedfeed == "tous":
        articles = base_query.filter(Posts.status == "plan")
    else:
        articles = base_query.filter(Networks.name == selectedfeed).filter(
            Posts.status == "plan"
        )
...
def record_new_post(form_data, image_file):
    ...
    network_id = (
        db.session.query(Networks.id).filter(Networks.name == network).first()
    )[0]
...
def update_post(post_id, title, description, tagline, post_datetime, network):
    ...
    post_to_modify = db.session.execute(
        db.select(Posts).filter_by(id=post_id)
    ).scalar_one()
    ...
    post_to_modify.network = (
        db.session.query(Networks.id).filter(Networks.name == network).scalar()
    )
    db.session.commit()
...
def create_article(article_data):
    ...
    try:
        ...
        db.session.rollback()
        logging.warning(
            "Impossible d'enregistrer dans la db l'article %s : %s", article.title, e
        )
        return None
...
def extract_data_from_html(html_article, url):
    article_data = {}
    try:
        article_data["title"] = html_article.find(
            "meta", attrs={"name": "twitter:title"}
        ).attrs["content"]
        try:
            article_data["image_url"] = html_article.find(
        ...
        article_data["link"] = html_article.find(
            "meta", attrs={"name": "twitter:url"}
        ).attrs["content"]
...
def update_article(article_data) -> bool:
    ...
    article_to_update = db.session.execute(
        db.select(Articles_rss).filter_by(id=article_data["id"])
    ).scalar_one()
    ...
    try:
        ...
        db.session.rollback()
        logging.error(
            "Erreur lors de la mise à jour de l'article %s : %s",
            article_data["title"],
            e,
        )
        return False
...
def new_post():
    ...
    if request.files:
        logging.debug("Fichiers reçus : %s", list(request.files.keys()))
    else:
        logging.debug("Aucun fichier uploadé")
...
def login():
    if request.method == "POST":
        ...
        if user and check_password_hash(user.password, password):
            ...
            flash(
                f"Connexion de {username} {'(admin)' if user.is_admin else '(utilisateur)'}"
            )
...
def import_link():
    ...
    logging.info(
        "Lien importé : %s pour le journal %s - Data %s", link, newspaper, data
    )
    if link:
